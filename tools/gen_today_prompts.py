...
def main():
    ...
    prompt_tpl_path = os.path.join(BASE_DIR, "prompts", "ai_prisoner_prompt.md")
    if not os.path.exists(prompt_tpl_path):
        print(f"❌ 错误: 未能在 prompts 目录下找到常规通用 Prompt 模板 [prompts/ai_prisoner_prompt.md]！")
        sys.exit(1)
    ...
    copilot_tpl_path = os.path.join(BASE_DIR, "prompts", "copilot_observer_prompt.md")
    if not os.path.exists(copilot_tpl_path):
        print(f"❌ 错误: 未能在 prompts 目录下找到 Copilot 专属 Prompt 模板 [prompts/copilot_observer_prompt.md]！")
        sys.exit(1)
    ...
    # 输出双轨拼接 Prompt 文件至 prompts/ 目录，保持根目录极致整洁
    output_prompts_file = os.path.join(BASE_DIR, "prompts", "today_prompts.txt")
    ...
    print(f"==================================================")
    print(f"💡 开箱即用运行指南：")
    print(f"   1. 打开 prompts/today_prompts.txt。")
    print(f"   2. 复制【复制区 A】投喂给 7 个不受限模型。")
    print(f"   3. 复制【复制区 B】投喂给 Copilot。")
    print(f"   4. 将收集到的所有 JSON 回复追加粘贴到 [.scratch/ai_replies.txt] 中。")
    print(f"   5. 运行 tools/merge_ai_responses.py 和 tools/import_daily_data.py 自动走完今日流水线！")
    print(f"==================================================")
